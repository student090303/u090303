<?php
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true) die();
/** @var CBitrixComponent $this */
/** @var array $arParams */
/** @var array $arResult */
###Инициализация глобальных переменных Битрикс###
global $DB;
/** @global CUser $USER */
global $USER;
###
\Bitrix\Main\Loader::includeModule('miet.kpi');
use MIET\KPI;
###Проверка входных параметров на корректность и приведение их к нужному виду###
$arParams["USER_ID"] = intval($arParams["USER_ID"]);
if(!$arParams["USER_ID"]) {
	ShowError(GetMessage("EMPTY_USER"));
	@define("ERROR_404", "Y");
	if($arParams["SET_STATUS_404"]==="Y") {
		CHTTP::SetStatus("404 Not Found");
	}
	return array();
}
$arParams["DATE_FORMAT"] = trim($arParams["DATE_FORMAT"]);
if(strlen($arParams["DATE_FORMAT"]) <= 0) {
	$arParams["DATE_FORMAT"] = $DB->DateFormatToPHP(CSite::GetDateFormat("SHORT"));
}
###
###Сохранение значений KPI###
if($_REQUEST['saveKPI']) {
	if(KPI\KPIManager::SetKPIEmployee($arParams["USER_ID"], $_REQUEST['UF_PERIOD'], $_REQUEST['KPI'])) {
		$arResult['OK'] = 'Изменения успешно сохранены';
	} else {
		$arResult['ERROR'] = 'Ошибка при сохранении';
	}
}
###
###Получение данных из БД###
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"] === "N" ? false: $USER->GetGroups()), date('Y')))) {
	if ($arUser = CUser::GetByID($arParams["USER_ID"])->Fetch()) {
		// Получаем имя пользователя
		$arResult['FIO_USER'] = CUser::FormatName(CSite::GetNameFormat(false), $arUser);
		###Формирование списка отчетных периодов (месяц, год)###
		//Получение текущего месяца и года
		for ($i = 1; $i <= 12; $i++) {
			if (strlen($i) == 1) {
				$i = '0' . $i;
			}
			$arResult['PERIOD_ITEMS'][] = array('VALUE' => '01.' . $i . '.' . date('Y'));
		}
		###
		###Получение списка показателей KPI для сотрудника###
		$arResult['ITEMS'] = KPI\KPIManager::GetKPIEmployee($arParams["USER_ID"]);
		###Кэширование значения элементов массива $arResult###
		$this->SetResultCacheKeys(array("ITEMS", "FIO_USER", "NAME", "PERIOD_ITEMS"));
		$this->EndResultCache();
	} else {
		$this->AbortResultCache();
		ShowError(GetMessage("USER_NOT_FOUND"));
		return;
	}
###
}

if(!$_REQUEST['UF_PERIOD']) {
	$_REQUEST['UF_PERIOD'] = '01.' . date('m') . '.' . date('Y');
}

// Т.к. Данные KPI пишутся в highload блок, а с ним тегированне кеширование не работает, то получать будем вне кешированной части.
// Да и файлов кеша получилось бы слишком много. По 12 на каждого пользователя.
// В лабораторной 5 мы создавали метод для получения данных о KPI. Будем использовать его. Немного модифицировав (в результаты добавим получение ID KPI и получение всех KPI (fetchAll()), а не одного (fetch()))
$arKpiIds = array_column($arResult['ITEMS'], 'ID');
$arKPI = KPI\KPIManager::GetKPIEmployeeValue($arKpiIds, $arParams['USER_ID'], $_REQUEST['UF_PERIOD']);

$arResult['KPI'] = array_combine(array_column($arKPI, 'UF_KPI'), $arKPI);

###Подключение шаблона компонента###
$this->IncludeComponentTemplate();
###
if(count($arResult["KPI"])) {
	return $arResult["KPI"];
}
###